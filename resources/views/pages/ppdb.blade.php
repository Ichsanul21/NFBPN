        ...
        <form method="POST" action="{{ route('ppdb.store') }}" enctype="multipart/form-data" class="mt-6" x-data="ppdbWizard(@js($conditionsByJenjang), @js(old('answers', [])), @js($periodsByJenjang), '{{ old('period_id') }}')">
            ...
            <div data-step="1" x-show="step === 1" class="mt-6">
                ...
                <div class="mt-3 grid sm:grid-cols-2 gap-4">
                    ...
                    <label class="grid gap-1.5 text-sm font-bold">Kata sandi (min. 8 karakter, campur besar-kecil, angka, simbol)<input id="ppdb-password" name="password" required type="password" minlength="8" autocomplete="new-password" class="font-normal rounded-xl border border-nf-blue/25 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-nf-blue">
                        @include('partials.password-meter', ['input' => 'ppdb-password', 'confirm' => 'ppdb-password-confirmation'])
                    </label>
                    <label class="grid gap-1.5 text-sm font-bold sm:col-span-2">Ulangi kata sandi<input id="ppdb-password-confirmation" name="password_confirmation" required type="password" minlength="8" autocomplete="new-password" class="font-normal rounded-xl border border-nf-blue/25 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-nf-blue"></label>
                </div>
                @else
